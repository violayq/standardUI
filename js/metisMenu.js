...
(function (global, factory) {
  ...
  var MetisMenu = function ($) {
    ...
    function createMenu(target, data, id, j) {
      ...
      $.each(rootMenu, function (i) {
        ...
        var li_a = $('<a/>').attr({"href": this.url || "#", "menuId": this.id})
            .addClass(this.url == '' ? '' : 'J_menuItem')
            .appendTo(li);
        // iconCls
        $('<i/>').addClass(this.iconClass || "").appendTo(li_a);
        // title
        $('<span class="nav-label" />').text(this.title).appendTo(li_a);

        var childrenUL = $('<ul class="nav" />');
        createMenu(childrenUL, data, that.id, j+1);
        if(childrenUL.children().length){
          if(j == 2) {
            childrenUL.addClass('nav-second-level');
          } else if(j == 3){
            childrenUL.addClass('nav-three-level');
          }
          childrenUL.appendTo(li);
          li_a.attr('href','#');
          li_a.removeClass('J_menuItem');
          // arrow
          $('<span class="fa arrow" />').appendTo(li_a);
        }
      });
    }

    function menuEvent(target){
      $(target._element).on('click', ".J_menuItem", function () {
        var arr = [];
        var url = $(this).attr('href');
        var title = $.trim($(this).text());
        arr.push(title);
        $(this).parent().siblings('li.active').removeClass('active');
        $('.J_menuItem').parent().removeClass('actived');
        $(this).parent().addClass('actived');
        if (url != null && url != '') {
          //添加动画效果
          $(target._config.container).addClass('animated');
          setTimeout(function () {
            $(target._config.container).removeClass('animated');
          }, 1000);
          ...
          $.ajax({
            ...
            success: function (res) {
              var resObj = $(res);
              $.parser.parse(resObj)
              $(target._config.container).empty();
              $(target._config.container).append(resObj);
            }
          });
        }
        target._config.onSelect.call($(this).parent()[0]);
        return false;
      });
    }
      ...
      _createClass(MetisMenu, [{
        ...
        value: function init() {
          if(this._config.data && this._config.data.length){
              $(this._element).empty();
              createMenu($(this._element), this._config.data, "0", 2);
          }
          menuEvent(this);
          var self = this;
        ...
        value: function _show(element) {
          ...
          _el.parent('li').siblings('.actived').removeClass('actived');
